...
def get_optimized_path(config, model, model_starttime):
    if len(config.experiment_suff) > 0:
        unique_folder = f'{config.data}_{model}_{config.experiment_suff}_{model_starttime}'
    else:
        unique_folder = f'{config.data}_{model}_{model_starttime}'
    ...
    return os.path.join(
        unique_folder_path,
        model + config.experiment_suff + '_optimized.csv'
    )
...
def get_parser():
    ...
    parser.add_argument('--gpr_fit_path',
                        type=str, required=False,
                        help='Path to fit gpr molecules, objective csv')
    parser.add_argument('--opt_start_path',
                        type=str, required=False,
                        help='Path to molecules for starting the optimization search csv')
    parser.add_argument('--opt_lr', type=float, default=0.01,
    ...
    parser.add_argument('--opt_b2', type=float, default=0.999,
                        help='Beta2 for mol optimization')
    parser.add_argument('--opt_eps', type=float, default=1e-8,
                        help='Epsilon for mol optimization')
...
def opt_from_model(config, model, model_starttime):
    ...
    opt_config = opt_parser.parse_known_args(
        [model] + sys.argv[1:] +
    ...
    opt_config.data = config.data
    
    if config.use_selfies:
        opt_config.use_selfies = True
    else:
        opt_config.use_selfies = False
    
    dict1 = vars(config)
    dict2 = vars(opt_config)
    
    whole_config = dict1.copy()
    whole_config.update(dict2)
    whole_config = argparse.Namespace(**whole_config)
    
    opt_script.main(model, whole_config)
...
def main(config):
    ...
    model = config.model
    opt_from_model(config, model, save_time)
