...
class VAEPROPERTY(nn.Module):
    def __init__(self, vocab, config):
        ...
        # Property Predictor
        predictor_layers = nn.ModuleList()
    
        out_dims = config.p_d_h
        predictor_layers.append(
            nn.Linear(config.d_z, out_dims)
            )
        for p_i in range(1, config.p_n_layers):
            predictor_layers.append(nn.Linear(
                out_dims, 
        ...
        self.property_predictor = nn.ModuleList([
    ...
    def forward(self, x, y=None):
        ...
        return kl_loss, recon_loss, prop_loss
    ...
    def forward_prop_predictor(self, z, y):
        ...
        :param y: (n_batch, d_y) of floats, property y
        :return: (n_batch, preds) of floats, predicted property y
        :return: float, mse component of loss
        """
        
        z = self.predictor_layers(z)
        preds = self.predictor_fc(z)
