...
from moses.vae_property import VAEPROPERTY, VAEPROPERTYTrainer
from moses.dataset import get_dataset


class VAEPROPERTYUtils(object):
    def __init__(self, vocab, config, model_path):
        self.vocab = vocab
        self.config = config
        self.reg_prop_tasks = config.reg_prop_tasks
        
        # Load model
        self.model = VAEPROPERTY(vocab, config)
    ...
    def predict_prop_Z(self, z, standardized=True):
        ...
        if hasattr(self.config, 'reg_prop_tasks') and len(self.config.reg_prop_tasks) > 0:
            z = self.model.predictor_layers(z)
            preds = self.model.predictor_fc(z)             
            return preds
